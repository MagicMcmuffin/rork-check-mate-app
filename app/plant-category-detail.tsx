...
import { Wrench, Plus, Trash2, Edit2, FolderPlus, FileText, Calendar, X, Check, Upload, Download, Eye, File, Image as ImageIcon } from 'lucide-react-native';
...
export default function PlantCategoryDetailScreen() {
  ...
  const directItems = getCategoryPlantItems(categoryId);
  ...
  const handleSaveSubcategory = async () => {
    ...
    try {
      if (editingSubcategoryId) {
        await updatePlantCategory(editingSubcategoryId, subcategoryName);
        Alert.alert('Success', 'Subcategory updated successfully');
      } else {
        await addPlantCategory(subcategoryName, categoryId);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deletePlantCategory(subcategoryId);
  ...
  const handleSaveItem = async () => {
    ...
    try {
      if (editingItemId) {
        await updatePlantItem(editingItemId, {
          name: itemName.trim(),
  ...
  const handleSaveCertificate = async () => {
    ...
    try {
      await addPlantCertificate(selectedItemId, {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deletePlantCertificate(itemId, certificateId);
        ...
        {subcategories.length > 0 && (
          <>
            ...
            {subcategories.map(sub => {
              const items = getCategoryPlantItems(sub.id);
              return (
...
const styles = StyleSheet.create({
  ...
  actionButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 12, padding: 14 },
  actionButtonText: { fontSize: 14, fontWeight: '600' as const, color: '#ffffff' },
  sectionTitle: { fontSize: 18, fontWeight: '700' as const, marginBottom: 12, marginTop: 8 },
  subcategoryCard: { borderRadius: 12, marginBottom: 16, borderWidth: 1, overflow: 'hidden' },
  subcategoryHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 14 },
  subcategoryHeaderLeft: { flexDirection: 'row', alignItems: 'center', flex: 1 },
  subcategoryIcon: { width: 36, height: 36, borderRadius: 8, justifyContent: 'center', alignItems: 'center' },
  subcategoryInfo: { marginLeft: 10, flex: 1 },
  subcategoryName: { fontSize: 15, fontWeight: '600' as const, marginBottom: 2 },
  subcategoryCount: { fontSize: 12 },
  subcategoryActions: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  iconButton: { width: 28, height: 28, borderRadius: 6, justifyContent: 'center', alignItems: 'center' },
  subcategoryItems: { padding: 14, paddingTop: 0, gap: 12 },
  itemCard: { borderRadius: 10, padding: 14, borderWidth: 1 },
  ...
  itemName: { fontSize: 15, fontWeight: '600' as const, flex: 1 },
  itemWarningBadge: { backgroundColor: '#fef3c7', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  itemWarningText: { fontSize: 11, fontWeight: '600' as const },
  itemActions: { flexDirection: 'row', gap: 6 },
  itemActionButton: { width: 28, height: 28, borderRadius: 6, justifyContent: 'center', alignItems: 'center' },
  itemDetails: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 10 },
  ...
  itemNotes: { fontSize: 12, fontStyle: 'italic' as const, marginBottom: 10 },
  certificatesSection: { marginTop: 12, paddingTop: 12, borderTopWidth: 1 },
  certificatesHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 10 },
  certificatesTitle: { fontSize: 13, fontWeight: '600' as const },
  certificatesList: { gap: 8 },
  certificateItem: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 8 },
  certificateIcon: { width: 32, height: 32, borderRadius: 6, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0, 122, 255, 0.1)' },
  certificateInfo: { flex: 1 },
  certificateName: { fontSize: 13, fontWeight: '600' as const, marginBottom: 6 },
  certificateMeta: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  certificateExpiry: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  certificateExpiryText: { fontSize: 11 },
  expiryBadge: { paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  expiryBadgeText: { fontSize: 10, fontWeight: '600' as const },
  certificateActions: { flexDirection: 'row', gap: 6 },
  certActionButton: { width: 28, height: 28, borderRadius: 6, justifyContent: 'center', alignItems: 'center' },
